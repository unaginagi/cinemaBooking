...
public class ReportBookingGUI_Weekly extends JFrame implements ActionListener {
    ...
    public ReportBookingGUI_Weekly() {
         setTitle("Weekly Booking Report");
        ...
        if (e.getSource() == generateButton) {
           ...
           // Check if the difference between start and end date is exactly 7 days
            if (startDate.plusDays(7).isEqual(endDate)) {
                // Perform logic for generating weekly booking report
                 String startDateString = startDate.toString();
            String endDateString = endDate.toString();
            System.out.println("Start: " + startDateString + "; End: "+ endDateString);
            weeklyReport = Controller.GenerateReportBookingWeeklyController.generateReportBookingWeeklyController(startDateString, endDateString);
